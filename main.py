...
def is_obstacle(depth_map, ground_map):
    ...
    return res
'''
focal = 554.254691191187
f = np.loadtxt('../../txt/60.txt', dtype='float32')
w = 640
h = 480
normal_map = cal_normal(f, smooth='off')

ground = ground_detection(normal_map)
cv2.imwrite('ground.png', ground*255)

cv2.normalize(normal_map, normal_map,0,1,cv2.NORM_MINMAX)
cv2.imwrite('snormal.png', normal_map*255)
cv2.normalize(f,f,0,1,cv2.NORM_MINMAX)
cv2.imwrite('test.png', f*255)
'''
for i in range(1,80):
    f = np.loadtxt('../../txt/%d.txt'%i, dtype='float32')
    normal_map = cal_normal(f, smooth='off')
    ground = ground_detection(normal_map)
    obs = is_obstacle(f, ground)
    cv2.imwrite('./obs/%d.png'%i, obs)
    #cv2.imwrite('./ground/%d.png'%i, ground*255)
    #cv2.normalize(normal_map, normal_map, 0, 1, cv2.NORM_MINMAX)
    #cv2.imwrite('./normal_no_smooth/%d.png'%i, normal_map*255)
    #cv2.normalize(f,f,0,1,cv2.NORM_MINMAX)
    #cv2.imwrite('./depth_map/%d.png'%i, f*255)
